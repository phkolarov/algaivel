...
class TagsRepository
{
    ...
    public function findAll()
    {
        ...
        $this->query = "SELECT * FROM tags" . $this->where . $this->order;
        $result = $db->prepare($this->query);
        $result->execute($this->placeholders);
        $collection = [];
        foreach ($result->fetchAll() as $entityInfo) {
            $entity = new Tag($entityInfo['nameBG'],
$entityInfo['name'],
$entityInfo['id']);
            $collection[] = $entity;
    ...
    {
        $db = Database::getInstance('app');
        $this->query = "SELECT * FROM tags" . $this->where . $this->order . " LIMIT 1";
        $result = $db->prepare($this->query);
        $result->execute($this->placeholders);
...
$entityInfo['id']);
    ...
    {
        $db = Database::getInstance('app');
        $this->query = "DELETE FROM tags" . $this->where;
        $result = $db->prepare($this->query);
        $result->execute($this->placeholders);
        return $result->rowCount() > 0;
    }
    public static function add(Tag $model)
    {
        if ($model->getId()) {
    ...
    private static function update(Tag $model)
    {
        $db = Database::getInstance('app');
        $query = "UPDATE tags SET nameBG= :nameBG, name= :name WHERE id = :id";
...
':name' => $model->getName()
    ...
    {
        $refc = new \ReflectionClass('\Models\Tag');
    ...
    public function pagination($pageNum,$count){
        ...
        $db = Database::getInstance('app');
        $this->query = "SELECT * FROM tags" . $this->where. " ORDER BY post_date DESC LIMIT $param1,$count;";
        $result = $db->prepare($this->query);
        $result->execute($this->placeholders);
        $collection = [];
        foreach ($result->fetchAll() as $entityInfo) {

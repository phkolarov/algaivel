...
use Core\Database;
use Models\Role;
use Collections\RoleCollection;
...
':role' => $model->getRole()
    ...
    {
        $refc = new \ReflectionClass('\Models\Role');
    ...
    public function pagination($pageNum,$count){
        ...
        $db = Database::getInstance('app');
        $this->query = "SELECT * FROM roles" . $this->where. " ORDER BY post_date DESC LIMIT $param1,$count;";
        $result = $db->prepare($this->query);
        $result->execute($this->placeholders);
        $collection = [];
        foreach ($result->fetchAll() as $entityInfo) {
